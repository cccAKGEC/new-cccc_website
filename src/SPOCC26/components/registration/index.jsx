...
const RegistrationMain = () => {
  ...
  useEffect(() => {
    ...
    if (!isMobileViewport || mobileView === 'branding') {
      return undefined;
    }
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name === 'name' || name === 'studentNumber') {
        ...
        const firstName = (nameVal.trim().split(/\s+/)[0] || '').toLowerCase().replace(/[^a-z]/g, '');
        const stdNo = stdVal.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
      ...
      setErrors((prevErr) => {
        ...
        } else if ((name === 'name' || name === 'studentNumber') && !updatedData.email) {
  ...
  return (
    <div className="relative min-h-screen w-full antialiased overflow-x-hidden selection:bg-[#00d2ff]/30 cursor-none lg:cursor-none text-white">
      ...
      <div className="relative z-10 w-full min-h-screen flex items-center justify-center py-0 lg:py-2">
        <div className={`w-full max-w-[440px] lg:max-w-[1360px] xl:max-w-[1440px] mx-auto px-4 md:px-8 pt-0 ${mobileView === 'branding' ? 'pb-0' : 'pb-16'} lg:pb-6 flex flex-col items-center justify-center min-h-[100dvh] lg:min-h-0 text-white relative z-10`}>
          {/* Toast Alert Banner */}
          {toast.show && (
          ...
          <div className="w-full h-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 xl:gap-20 items-center justify-center transition-all duration-300">
