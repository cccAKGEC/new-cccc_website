...
const FormField = ({
  ...
  const baseInputStyles = `
    w-full px-3.5 sm:px-5 lg:px-6 py-3 sm:py-3.5 lg:py-4 bg-transparent rounded-xl sm:rounded-2xl text-white font-inter font-normal placeholder-[#5b6e9c]
    transition-all duration-300 outline-none text-[13px] sm:text-[15px] md:text-[16px] lg:text-[17px] tracking-wide relative z-10 text-ellipsis overflow-hidden
    ${error
  ...
  const GradientBorder = ({ isActive }) => (
    <div
      className={`absolute inset-0 pointer-events-none rounded-2xl transition-all duration-300 z-0 ${!isActive ? 'animate-input-border' : ''}`}
      style={{
      ...
      {isSelect ? (
        <div className="relative" ref={dropdownRef}>
          <div
            ...
            className={`${baseInputStyles} flex items-center justify-between cursor-pointer select-none ${!value ? 'text-[#5b6e9c]' : 'text-white'
              }`}
          >
